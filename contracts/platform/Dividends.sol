...
contract IdoruDividends is Context, Ownable {
  ...
  function payDividends(uint256 blockPay, uint256 amount) public onlyOwner {
  ...
  function withdrawDividendsBlock(uint256 blockWithdraw)
    ...
    returns (uint256)
  {
    uint256 pastTotalSupply = IIdoru(_idoruToken).getPastTotalSupply(
      blockWithdraw
    ); // ?
    uint256 toWithdraw = ((dividendsAmounts[blockWithdraw] +
      dividendsPaid[blockWithdraw]) *
      IIdoru(_idoruToken).minHoldingValue(msg.sender, blockWithdraw)) /
      pastTotalSupply;
    require(dividendsAmounts[blockWithdraw] > toWithdraw, "Not enough money");
    return toWithdraw;
  }

  function withdrawAllDividendsView() public view returns (uint256) {
  ...
  function withdrawAllDividendsChangeState() private returns (uint256) {
    require(dividendBlocks.length > 0, "No blocks with dividends");
  ...
  function claimDividends() private returns (uint256) {
    IERC20 stableERC20 = IERC20(_stablecoinToken);
